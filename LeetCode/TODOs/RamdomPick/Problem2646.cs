    ...
    internal class Problem2646
    {
        public class Solution
        {
            ...
            public int MinimumTotalPrice(int n, int[][] edges, int[] price, int[][] trips)
            {
                ...
                {
                    CountNodeInPath(n, connected, trip[0], trip[1], nc);
                }

                bool[] visited = new bool[n];
                for (int i = 0; i < n; i++) { visited[i] = false; }
                Solve(n, connected, price, 0, nc, visited);
                ...
                return ans - Math.Max(dp[0].Selected, dp[0].NotSelected);
            }
            ...
            private void Solve(int n, List<int>[] connnected, int[] price, int node, int[] nc, bool[] visited)
            {
                ...
                foreach (var next in connnected[node])
                {
                    ...
                    Solve(n, connnected, price, next, nc, visited);

                    selected += dp[next].NotSelected;
                    notSelected += Math.Max(dp[next].Selected, dp[next].NotSelected);
                }

                dp[node] = (selected, notSelected);
            }
            ...
            private void CountNodeInPath(int n, List<int>[] connnected, int start, int end, int[] nc)
            {
                bool[] visited = new bool[n];
                for (int i = 0; i < n; i++) { visited[i] = false; }
                CountPath(connnected, start, end, nc, visited);
                nc[start]++;
            }
            ...
            private bool CountPath(List<int>[] connnected, int start, int end, int[] nc, bool[] visited)
            {
                ...
                foreach (var next in connnected[start])
                {
                    ...
                    if (CountPath(connnected, next, end, nc, visited))
                    {
                        nc[next]++;
                        return true;
                    }
